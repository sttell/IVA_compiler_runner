...
inline bool is_cmd_file(std::string filename) {
    return filename.find("cmd") != std::string::npos;
}
...
public:
    ...
    explicit ProgramComparatorModule(const GlobalSettings& _settings) : 
        settings(_settings.program_comparator) { throw_desc = PROGRAM_COMAPRATOR_THROW_DESC; };
