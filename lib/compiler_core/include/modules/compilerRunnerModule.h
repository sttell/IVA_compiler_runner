...
#define RUN_COMPILE_SCRIPT_PATH "/src/compiler_runner_scripts/runner.sh "
...
public:
    // Конструктор
    explicit CompilerRunnerModule(const GlobalSettings& _settings, StatusBufferCache* status_bar) :
       Module(status_bar, COMPILER_RUNNER_THROW_DESC), settings(_settings.compiler_runner) {};
    ~CompilerRunnerModule() {log=nullptr; delete log;};
