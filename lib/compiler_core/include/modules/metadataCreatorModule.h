...
public:
    ...
    explicit MetadataCreatorModule(const GlobalSettings& _settings, StatusBufferCache* status_bar) :
        Module(status_bar, METADATA_CREATOR_THROW_DESC), settings(_settings.metadata_creator) {};
    
    ~MetadataCreatorModule() {log=nullptr; delete log;};
...
private:
    ...
    void prepareWieghtsFile(std::string in, std::string out) const;
