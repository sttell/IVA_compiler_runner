...
exit_module_status MetadataCreatorModule::runProcess() {
    ...
    try {
        log->writeMessage("Проверка настроек модуля.");
        // Проверяем настройки модуля на корректность значений
        checkSettingsCorrectness();
        log->writeMessage("Настройки корректны.");
        ...
        log->writeMessage("Перекладывание весов hpm_weights.bin -> hpm_coeff.");
        prepareWieghtsFile(
            (settings.compiler_dir + "hpm_weights.bin").c_str(),
            (settings.compiler_dir + "hpm_coeff").c_str()
        );
        log->writeMessage("Перекладывание прошло успешно.");
        ...
        log->writeMessage("Проверка корректности дескрипторов файлов программы.");
        checkProgramFileDescriptorsCorrectness(file_descriptors);
        log->writeMessage("Проверка завершена успешно.");
...
void MetadataCreatorModule::checkSettingsCorrectness() const {
    ...
    // Проверка существования директории ко мпилятора
    checkDirExist(settings.compiler_dir);

    // Проверка наличия весов
    checkFileExist(settings.compiler_dir + "hpm_weights.bin");
};


// Подготовка весов для программы
void MetadataCreatorModule::prepareWieghtsFile(std::string in, std::string out) const {
    ...
    if (return_code != 0 && !(stderr_output.empty())) {
        std::string err_desc("Ошибка предварительной подготовки весов: ");
...
void MetadataCreatorModule::checkMapSizeCorrect(const MapSize& map) const {
    ...
    if (map.B < 1 || map.L < 1 || map.X < 1 || map.Y < 1) {
        throw std::runtime_error("Размеры осей карты признаков не могут быть меньше 1.");
    }
};
...
void MetadataCreatorModule::checkProgramFileDescriptorsCorrectness(
    ...
    if (count_of_not_exists_files > 0) {
        ...
        for (const auto& file_desc : file_descriptors) {
            if (!file_desc.is_exist && file_desc.filename.find("hpm_coeff") != std::string::npos)
...
void MetadataCreatorModule::createConstantsBlock(
    ...
    if (constants_descriptors.empty())
        throw std::runtime_error("Внутренняя ошибка модуля. Проверьте исходные файлы.");
